...
@dlt.table(
    name="jp_assessment.latam_lab_gold.report_sales_transaction",
    comment="Pivoted transactions per product aggregated by total sales per day")
...
@dlt.table(
    name="jp_assessment.latam_lab_gold.report_customer_360",
    comment="Detailed transactions enriched with customer info")
def transactions_details():
    df_trans = spark.table("samples.bakehouse.sales_transactions")
